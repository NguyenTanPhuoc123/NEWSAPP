...
class ControllerNews {
  ...
  static Future<void> getListAllNews(List<String> url) async {
    for (var urlFeed in url) {
      ...
      for (var item in feed.items!) {
        ...
        var logoUrl = feed.image!.url != null
            ? feed.image!.url.toString()
  ...
  static List<News> getListNewsByOfficial(String official) {
    List<News> listNewsByOfficial = List.filled(
        0, News("", "", List.empty(), "", "", "", "", ""),
        growable: true);
  ...
  static loadListNewsFromJson(String key) {
    ...
    listNewsCollection =
        lst?.map((news) => News.fromJson(json.decode(news))).toList() ?? [];
  }
  ...
  static Future<void> saveNewsFavorite(String username,News news) async{
    await databaseRef.child("FavoriteNews").child(username).child(news.title).set(news.toJson());
  }
  ...
  static Future<void> removeNewsFavorite(String username, News news) async{
    await databaseRef.child("FavoriteNews").child(username).child(news.title).remove();
  }

  static Future<List<String>> getUserLiked(String title) async{
    final listUsersLiked = List.filled(0,"",growable: true);
    final snapshot = await databaseRef.child("FavoriteNews").get();
  ...
  static Future<void> saveNewsRead(String username,News news) async{
    await databaseRef.child("History").child(username).child(news.title).set(news.toJson());
  }
